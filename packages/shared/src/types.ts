...
/**
 * One piece of root-cause evidence explaining "why this incident might
 * be happening". Generated from existing data (HealthIncident pool +
 * IncidentHistoricalContext) — no ML, no external API.
 *
 * `confidence` is a deterministic 0..1 score reflecting how much
 * supporting data exists (e.g. "history evidence" confidence rises
 * with occurrenceCount). NOT a probability — it's a coverage signal.
 */
export type RootCauseEvidenceKind =
  | 'history'    // Same-kind prior history (recurrence, recovery rate)
  | 'severity'   // Current severity + escalation history
  | 'impact'     // Affected executions / agents breadth
  | 'agent'      // Agent-level recurrence / degradation pattern
  | 'trend'      // Recovery trend over time
  | 'priority';  // Priority / signal context for this kind

export interface RootCauseEvidenceItem {
  /** Stable category so the UI can group / color. */
  kind: RootCauseEvidenceKind;
  /** Human-readable explanation (English). */
  message: string;
  /**
   * Confidence in [0, 1]. Higher = more supporting data.
   * Computed deterministically from the underlying counts.
   */
  confidence: number;
  /**
   * Optional advisory weight 0..1 — UI sort key for "most informative
   * evidence first". Always <= confidence.
   */
  weight: number;
}

/**
 * Root-cause evidence bundle for a single HealthIncident.
 *
 * Generated by `buildRootCauseEvidence()` from the current incident +
 * its IncidentHistoricalContext + (optionally) the matching priority
 * insight + a same-execution / same-agent rollup.
 *
 * Pure-derived; deterministic; no DB writes. Returned by
 * `GET /api/incidents/:incidentKey/evidence`.
 */
export interface IncidentRootCauseEvidence {
  /** The incidentKey this evidence is for (echo). */
  incidentKey: string;
  /** Execution this incident belongs to (echo). */
  executionId: string;
  /** Anomaly kind (echo). */
  kind: HealthAnomalyKind;
  /** Ordered evidence items (descending by weight, then confidence). */
  evidence: RootCauseEvidenceItem[];
  /** Overall confidence = max(individual.confidence). 0 when no evidence. */
  confidence: number;
  /** True when at least one evidence item was generated. */
  hasEvidence: boolean;
...
/**
 * Action type suggested for the user to take next. These are pure
 * deterministic suggestions derived from the IncidentInvestigationReport —
 * NEVER auto-executed, NEVER mutated, NEVER side-effected.
 *
 * The UI shows these as "Suggested next steps"; humans decide whether
 * to act on them.
 */
export type IncidentRecommendedActionType =
  | 'inspect-agent'      // Agent has recurring degradation pattern — investigate the agent
  | 'review-execution'   // Multiple executions affected — review each one
  | 'compare-history'    // Kind has occurred many times — compare with prior incidents
  | 'watch-recurrence';  // Recovery is unstable / recurrence is high — monitor closely

/**
 * Action priority bucket. Sorted DESC when rendered (high → medium → low).
 */
export type IncidentRecommendedActionPriority = 'high' | 'medium' | 'low';

/**
 * One recommended action item — deterministic suggestion derived from
 * the IncidentInvestigationReport. Each action carries:
 *   - `type`     — stable enum so the UI can icon / group
 *   - `priority` — advisory bucket (high / medium / low)
 *   - `reason`   — human-readable explanation (English, single sentence)
 *
 * Returned by `GET /api/incidents/:incidentKey/actions`. Pure-derived;
 * deterministic; no DB writes; no ML.
 */
export interface IncidentRecommendedAction {
  /** Stable action kind so the UI can group / icon. */
  type: IncidentRecommendedActionType;
  /** Advisory priority bucket. */
  priority: IncidentRecommendedActionPriority;
  /** Human-readable single-sentence reason. */
  reason: string;
}

/**
 * Recommended-action bundle for a single HealthIncident.
 *
 * Returned by `GET /api/incidents/:incidentKey/actions`. Generated by
 * `buildRecommendedActions(report)` — pure aggregation over the
 * three views in IncidentInvestigationReport.
 */
export interface IncidentRecommendedActionBundle {
  /** The incidentKey these actions are for (echo). */
  incidentKey: string;
  /** Ordered actions (priority DESC, type ASC). */
  actions: IncidentRecommendedAction[];
  /** True when at least one action was suggested. */
  hasActions: boolean;
...
/**
 * One timeline event in a HealthIncident's investigation lifecycle.
 *
 * Derived deterministically from existing data (current incident +
 * historical context) — no LLM, no scheduler, no real-time polling.
 *
 * Stable category so the UI can group / color / icon.
 */
export type IncidentInvestigationTimelineEventType =
  | 'detected'     // First detection of this incident
  | 'escalated'    // Severity escalated (high → critical)
  | 'recovered'    // Incident was resolved
  | 'recurred';    // A prior occurrence of the same kind happened

/**
 * One row in an Investigation Timeline. Emitted in timestamp-ASC order
 * (with type-ASC as a stable tie-break). Pure-derived from the
 * IncidentInvestigationReport fields.
 */
export interface IncidentInvestigationTimelineEvent {
  /** ISO timestamp of this event. */
  timestamp: string;
  /** Stable event type so the UI can group / color. */
  type: IncidentInvestigationTimelineEventType;
 ...
 *
 * Generated by `buildInvestigationTimeline(report)` — pure aggregation
 * over the IncidentInvestigationReport (v1.15) fields:
 *   - `detected`  ← current.detectedAt (always)
 *   - `escalated` ← current.escalationCount > 0
 *   - `recovered` ← current.lifecycle === 'recovered'
 *   - `recurred`  ← one event per entry in report.history.previousIncidents
 *
 * Pure / read-only / deterministic. Returned by
 * `GET /api/incidents/:incidentKey/timeline`.
 */
export interface IncidentInvestigationTimeline {
  /** The incidentKey this timeline is for (echo). */
  incidentKey: string;
  /** Ordered events (timestamp ASC, type ASC). */
  events: IncidentInvestigationTimelineEvent[];
  /** When this timeline was assembled (caller-supplied nowIso). */
  generatedAt: string;
}